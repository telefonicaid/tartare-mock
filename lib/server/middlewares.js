...
var schemas = require('./json-schemas');
...
function _buildRequestObject(req, config, cb) {
  ...
  requestObj.chunked = req.headers['transfer-encoding'] ?
    ...
    admin: {
      configs: {
        create: function create(req, res, next) {
          ...
          if (bodyErrs.length) {
            res.statusCode = 400;
            var errStacks = bodyErrs.map(function(err) { return err.stack; });
            return next(new Error('Bad formed body [' + errStacks.join(' | ') + ']'));
          }
      ...
      lastRequests: {
        readCollection: function readCollection(req, res, next) {
          context.lastRequests.find(req.query).sort({timestamp: 1}).exec(function(err, lastRequestList) {
            if (err) {
              res.statusCode = 500;
              return next(new Error('Database error [' + err + ']'));
            }
