...
var runServer = function runServer(settings, timeout, cb) {
  ...
  var mockProcess = cp.fork(path.resolve(__dirname, '../bin/mockserver'), args, {silent: true});
  ...
  var timeoutId = setTimeout(function() {
    ...
    mockProcess.kill();
    var err = new Error('The Mock Server couldn\'t be started before ' + timeout + ' milliseconds');
    err.stderr = stderr;
    err.stdout = stdout;
    cb(err);
  }, timeout);
  ...
  mockProcess.stdout.on('data', function(chunk) {
    ...
    // Look for the mock to have been started
    if (stdout.indexOf('Mock listening on ports:') !== -1) {
